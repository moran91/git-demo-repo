...
/**
 * A photo an owner posts to the explore feed. It is live for `POST_TTL_HOURS` from publishing, then
 * the client stops showing it and the scheduled sweep deletes it (documents and image). The id is
 * generated by the client so the photo can be uploaded under it before the post exists.
 */
export interface BranchPost {
  id: string;
  businessId: string;
  branchId: string;
  /** Storage path under businesses/{b}/branches/{br}/posts/{id}/. Required: a post is a photo. */
  imagePath: string;
  caption: Localized;
  /** Optional linked product of the same branch ("order this"). */
  productId?: string;
  createdAt: string;
  /** ISO timestamp, createdAt + POST_TTL_HOURS. */
  expiresAt: string;
}
export const POST_TTL_HOURS = 24;
/** Live (unexpired) posts per branch. */
export const MAX_LIVE_POSTS = 10;
/** Menu items a branch can feature in its stories at once. */
export const MAX_STORY_ITEMS = 10;
...
export interface Product {
  ...
  mostOrdered?: boolean;
  /** Plays in the store's story circle on the customers' home page (needs a photo to show). */
  inStories?: boolean;
  archived: boolean;
  sortOrder: number;
  createdAt: string;
