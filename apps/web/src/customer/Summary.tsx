 ...
 */
export function Summary({ totals, mode, cashReceived, rejected }: { totals: OrderTotals; mode: 'pickup' | 'delivery'; cashReceived?: number; rejected?: boolean }) {
  const t = useT();
  ...
  return (
    <div className="summary card">
      ...
      {rejected ? null : <div className="muted" style={{ textAlign: 'end' }}>{cashReceived !== undefined ? t('dash.cashRecorded', { amount: money(cashReceived, locale) }) : mode === 'delivery' ? t('checkout.cashOnDelivery') : t('checkout.cashOnPickup')}</div>}
