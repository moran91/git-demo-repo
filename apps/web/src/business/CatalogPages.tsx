  ...
  return (
    <div className="stack">
      <PageTitle title={t('dash.catalog')}>
        ...
        <Link className="btn btn--secondary btn--sm" to="extras"><Icon name="tag" size={16} /> {t('catalog.library')}</Link>
              ...
              {d.variants.map((v, i) => (
                <div key={v.id ?? i} className="card stack--sm stack">
                  <div className="edit-row edit-row--plain">
                    ...
                    <div className="edit-row__foot"><IconButton icon="trash" label={t('common.remove')} onClick={() => set({ variants: d.variants.filter((_, j) => j !== i) })} /></div>
                  </div>
                </div>
              ))}
            </section>
...
function ModifierGroupsSection({ groups, onChange }: { groups: GroupIn[]; onChange: (g: GroupIn[]) => void }) {
  ...
  const moveButtons = (i: number) => <><IconButton icon="arrow" label={t('catalog.moveUp')} disabled={i === 0} onClick={() => move(i, -1)} style={{ transform: 'rotate(-90deg)' }} /><IconButton icon="arrow" label={t('catalog.moveDown')} disabled={i === groups.length - 1} onClick={() => move(i, 1)} style={{ transform: 'rotate(90deg)' }} /></>;
  ...
  return (
    <section className="stack--sm stack">
      <div className="section-head">
        ...
        <div className="actions">
          {linkable.length ? (
            <Select label={t('catalog.addFromLibrary')} value="" onChange={(e) => { const s = byId.get(e.target.value); if (s) onChange([...groups, linkGroup(s, makeId(8), groups.length)]); }}>
              <option value="">{t('catalog.addFromLibrary')}</option>
              {linkable.map((g) => <option key={g.id} value={g.id}>{L(g.name, business.defaultLocale)}</option>)}
            </Select>
          ) : null}
          <Button size="sm" variant="secondary" icon="plus" onClick={() => onChange([...groups, { id: makeId(8), sortOrder: groups.length, ...newGroupDraft() }])}>{t('catalog.addGroup')}</Button>
        </div>
      </div>
      {groups.map((g, gi) => {
        ...
        if (g.sharedGroupId) {
          return (
            <div key={g.id ?? gi} className="card stack--sm stack">
              <div className="edit-row edit-row--plain">
                <div className="edit-row__main stack--sm stack">
                  <div className="row" style={{ gap: 6 }}><strong>{L(view.name, business.defaultLocale)}</strong><Badge tone="accent" icon="tag">{t('catalog.linkedGroup')}</Badge></div>
                  <div className="muted">{view.options.map((o) => `${L(o.name, business.defaultLocale)}${o.priceDeltaAgorot ? ` (${money(o.priceDeltaAgorot, locale)})` : ''}`).join(' · ')}</div>
                  <div className="muted">{t('catalog.linkedGroupHint')}</div>
                  <div className="actions">
                    {shared ? <Button size="sm" variant="secondary" icon="edit" onClick={() => setEditShared({ index: gi, shared })}>{t('catalog.editInLibrary')}</Button> : null}
                    <Button size="sm" variant="ghost" onClick={() => { const { sharedGroupId: _s, ...rest } = view; update(gi, rest); toast(t('catalog.detached')); }}>{t('catalog.detach')}</Button>
                  </div>
                </div>
                <div className="edit-row__foot"><div className="actions actions--icons">{moveButtons(gi)}<IconButton icon="trash" label={t('common.remove')} onClick={() => onChange(groups.filter((_, j) => j !== gi))} /></div></div>
              </div>
            </div>
          );
        }
        const draft: GroupDraft = { name: g.name, required: g.required, minSelect: g.minSelect, maxSelect: g.maxSelect, options: withIds(g.options), placement: g.placement };
        return (
          <div key={g.id ?? gi} className="card stack--sm stack">
            <ModifierGroupFields value={draft} onChange={(next) => update(gi, { ...g, ...next })} foot={<div className="actions actions--icons">{moveButtons(gi)}<IconButton icon="trash" label={t('common.remove')} onClick={() => onChange(groups.filter((_, j) => j !== gi))} /><IconButton icon="tag" label={t('catalog.saveToLibrary')} onClick={() => setPromote(gi)} /></div>} />
          </div>
        );
      })}
