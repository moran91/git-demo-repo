...
import { KvRow, SwitchRow } from './SettingsPages';
import './settings.css';
...
export function ReceiptPreview({ model }: { model: ReceiptModel }) {
  ...
  return (
    <div className="stack--sm stack">
      ...
      <p className="muted">{t('printers.paperValue', { mm: model.paperWidthMm, dots: model.printableDots })} · {LOCALE_NAMES[model.locale]}</p>
    </div>
  );
}
...
export function PrintersPage() {
  ...
  const addPrinter = () => { const d = defaultDraft(); if (support !== 'ok') { d.transport = 'os_print_dialog'; d.profileId = 'os_print_dialog'; } d.receiptLocale = locale; setEdit({ d }); };
  ...
  return (
    <div className="sx-page sx-page--wide">
      ...
      {printers.data.length === 0 && !printers.loading ? <EmptyState icon="printer" title={t('printers.noPrinters')} action={can('printers_config') ? <Button icon="plus" onClick={addPrinter}>{t('printers.add')}</Button> : undefined} /> : null}
      <div className="sx-grid">
        {printers.data.map((p) => {
          ...
          const on = isMine && station.connection ? true : online.length > 0;
          return (
            <section key={p.id} className="sx-card sx-card--tight" aria-labelledby={`pr-${p.id}`}>
              <div className="sx-card__head" style={{ marginBottom: 8 }}><h2 className="sx-card__title" id={`pr-${p.id}`} style={{ color: 'var(--color-text)', fontSize: 18 }}>{p.name}</h2>{p.setupVerified ? <Badge tone="success" icon="check">{t('printers.verified')}</Badge> : <Badge tone="accent">{t('printers.notVerified')}</Badge>}</div>
              <ul className="kv">
                <KvRow label={t('printers.transport')} value={t(`printers.transport.${p.transport}`)} />
                <KvRow label={t('printers.profile')} sub={profile && !profile.verified ? t('printers.profileCandidate') : undefined} value={<span className="wrap-anywhere" style={{ whiteSpace: 'normal', textAlign: 'end' }}>{profile?.label ?? p.profileId}</span>} />
                <KvRow label={t('printers.paper')} value={<span className="sx-num">{t('printers.paperValue', { mm: p.paperWidthMm, dots: p.printableDots })}</span>} />
                <KvRow label={t('printers.language')} value={LOCALE_NAMES[p.receiptLocale]} />
                <KvRow label={t('printers.role')} value={<span dir="ltr">{p.role}</span>} />
                <KvRow label={t('printers.autoPrint')} value={t(`printers.auto.${p.autoPrint}`)} />
              </ul>
              <div className="sx-row" role="status">
                <span className={`status-dot ${on ? 'status-dot--on' : 'status-dot--off'}`} />
                <div className="sx-row__text"><span className="sx-row__sub" style={{ color: 'inherit' }}>
                  ...
                  {isMine && station.status === 'printing' && station.progress ? <span> · {t('printers.printing')} <span className="sx-num">{station.progress.sent}/{station.progress.total}</span></span> : null}
                </span></div>
              </div>
              ...
              <div className="sx-actions" style={{ marginTop: 12 }}>
                {p.transport === 'web_bluetooth_ble' ? (isMine && station.connection ? <Button variant="secondary" icon="bluetooth" onClick={() => stopStation()}>{t('printers.disconnect')}</Button> : <Button icon="bluetooth" disabled={support !== 'ok'} onClick={() => connect(p)}>{t('printers.connect')}</Button>) : null}
                {p.transport !== 'os_print_dialog' ? <Button variant="secondary" icon="printer" onClick={() => testPrint(p)}>{t('printers.testPrint')}</Button> : null}
                {p.transport === 'os_print_dialog' ? <Button variant="secondary" icon="download" onClick={() => setOsModel(testModel(p))}>{t('printers.testPrint')}</Button> : null}
                <Button variant="secondary" icon="eye" onClick={() => setPreview(testModel(p))}>{t('printers.preview')}</Button>
                {can('printers_config') ? <Button variant={p.setupVerified ? 'ghost' : 'primary'} icon="check" onClick={() => setVerifyFor(p)}>{t('printers.verifySetup')}</Button> : null}
              </div>
              {can('printers_config') ? <div className="sx-row" style={{ borderTop: 0, justifyContent: 'flex-end' }}>
                <IconButton icon="edit" label={t('printers.edit')} onClick={() => { const { id: _i, businessId: _b, branchId: _br, setupVerified: _s, active: _a, createdBy: _c, createdAt: _ca, updatedAt: _u, ...d } = p; setEdit({ id: p.id, d }); }} />
                <IconButton icon="trash" label={t('common.delete')} onClick={() => setDeactivate(p.id)} />
              </div> : null}
              {p.transport === 'os_print_dialog' ? <p className="sx-card__sub">{t('printers.osHint')}</p> : null}
              {p.transport === 'android_rfcomm' ? <p className="sx-card__sub">{t('printers.transport.android_rfcomm')} — {t('printers.sendToStation')}</p> : null}
            </section>
          );
        })}
      </div>
      ...
      {stale.length > 0 ? (
        <section className="sx-card" aria-labelledby="pr-stale">
          <h2 className="sx-card__title" id="pr-stale">{t('printers.staleJobs')}</h2>
          <p className="sx-card__sub">{t('printers.staleHint')}</p>
          <div className="sx-fields">{stale.map((j) => <Checkbox key={j.id} label={`${j.template} · ${j.orderId ?? 'test'} · ${formatLocalDateTime(j.requestedAt, locale)}`} checked={selectedStale.includes(j.id)} onChange={(e) => setSelectedStale(e.target.checked ? [...selectedStale, j.id] : selectedStale.filter((x) => x !== j.id))} />)}</div>
          <div className="sx-actions">
            <Button disabled={selectedStale.length === 0 || !station.connection} onClick={async () => { for (const id of selectedStale) await printSpecificJob(id); setSelectedStale([]); }}>{t('printers.printSelected')}</Button>
            <Button variant="secondary" disabled={selectedStale.length === 0} onClick={async () => { for (const id of selectedStale) await call('resolvePrintJob', { jobId: id, resolution: 'cancelled', note: 'skipped stale' }).catch((e) => toast(t(errorKey(e)), 'danger')); setSelectedStale([]); }}>{t('printers.skipSelected')}</Button>
          </div>
        </section>
      ) : null}

      <section className="sx-card sx-card--tight" aria-labelledby="pr-queue">
        <h2 className="sx-card__title" id="pr-queue">{t('printers.queue')}</h2>
        {jobs.data.length === 0 ? <p className="sx-card__sub">{t('common.emptyGeneric')}</p> : null}
        <ul className="sx-rows">
          {jobs.data.map((j) => {
            const needsAction = j.state === 'sent_unconfirmed' || j.state === 'needs_review' || j.state === 'failed_before_send' || j.state === 'queued';
            return (
              <li key={j.id} className={`sx-row ${needsAction ? 'sx-row--wrap' : ''}`}>
                <div className="sx-row__text">
                  <span className="sx-row__title">{j.template === 'test' ? t('receipt.testReceipt') : j.template === 'order_ticket' ? t('receipt.orderTicket') : t('receipt.customerCopy')}{j.orderId ? <span className="muted" dir="ltr"> · {j.orderId.slice(0, 8)}</span> : null} {j.isReprint ? <Badge tone="accent">{t('printers.copyLabel')}</Badge> : null} {isReceiptModel(j.receipt) && j.receipt.labels.revised ? <Badge tone="accent">{t('printers.revisedLabel')}</Badge> : null}</span>
                  <span className="sx-row__sub"><bdi>{formatLocalDateTime(j.requestedAt, locale)}</bdi> · {j.trigger} · {j.printerRole}{j.lastError ? ` · ${errText(j.lastError) || j.lastError}` : ''}{j.progress ? <> · <span className="sx-num">{j.progress.stripsSent}/{j.progress.stripsTotal}</span></> : null}</span>
                </div>
                <div className="sx-row__end">
                  <Badge tone={jobTone(j.state)}>{t(`printers.state.${j.state}`)}</Badge>
                  {isReceiptModel(j.receipt) ? <IconButton icon="eye" label={t('printers.preview')} onClick={() => setPreview(j.receipt as ReceiptModel)} /> : null}
                </div>
                {needsAction ? <div className="sx-row__foot">
      ...
      <Dialog open={!!edit} onClose={() => setEdit(null)} title={edit?.id ? t('printers.edit') : t('printers.add')} footer={<><Button variant="secondary" onClick={() => setEdit(null)}>{t('common.cancel')}</Button><Button loading={saving} onClick={save}>{t('common.save')}</Button></>}>
        {edit ? (
          <div className="sx-fields">
            ...
            <p className="sx-card__sub">{getProfile(edit.d.profileId)?.notes}</p>
            ...
            <ul className="sx-rows"><SwitchRow icon="printer" title={t('printers.cut')} checked={edit.d.cutSupported} disabled={!getProfile(edit.d.profileId)?.cutSupported} onChange={(cutSupported) => setEdit({ ...edit, d: { ...edit.d, cutSupported } })} /></ul>
            <TextInput label={t('printers.feed')} type="number" ltr min={0} max={10} value={edit.d.feedLinesAfter} onChange={(e) => setEdit({ ...edit, d: { ...edit.d, feedLinesAfter: Number(e.target.value) } })} />
