...
export function LocalizedInput({ label, value, onChange, required, multiline, error }: { label: string; value: Localized; onChange: (v: Localized) => void; required?: boolean; multiline?: boolean; error?: string }) {
  ...
  const [primary] = useState(() => value[locale]?.trim() ? locale : LOCALES.find((l) => value[l]?.trim()) ?? locale);
  ...
  const renderRow = (l: (typeof LOCALES)[number]) => {
    const rowId = `${id}-${l}`;
    ...
    return <div key={l} className="localized__row"><label className="localized__lang" htmlFor={rowId}>{names[l]}</label>{multiline ? <textarea {...common} className="textarea localized__control" rows={2} onChange={(e) => onChange({ ...value, [l]: e.target.value })} /> : <input {...common} type="text" className="input localized__control" onChange={(e) => onChange({ ...value, [l]: e.target.value })} />}</div>;
  };
  return (
