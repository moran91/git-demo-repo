...
describe('most ordered + combos', () => {
  ...
  it('owner creates a promoted combo with quantities; weight items and duplicates are rejected; public projection follows active/archived', async () => {
    expect(await expectCode(owner1.call('saveCombo', { businessId: IDS.restaurant, branchId: IDS.branchA, combo: { name: { en: 'x' }, description: {}, items: [{ productId: 'p-fries', quantity: 1 }, { productId: 'p-fries', quantity: 2 }], discountPercent: 10, promoted: true, active: true } }))).toBe('invalid_argument');
    expect(await expectCode(owner1.call('saveCombo', { businessId: IDS.market, branchId: IDS.marketBranch, combo: { name: { en: 'x' }, description: {}, items: [{ productId: 'p-tomato', quantity: 1 }, { productId: 'p-milk', quantity: 1 }], discountPercent: 10, promoted: true, active: true } }))).toBe('forbidden'); // other business
    const r = await owner1.call<{ combo: { id: string } }>('saveCombo', { businessId: IDS.restaurant, branchId: IDS.branchA, combo: { name: { en: 'Two falafel + fries', he: 'שני פלאפל + צ׳יפס' }, description: {}, items: [{ productId: 'p-falafel', variantId: 'v-reg', quantity: 2 }, { productId: 'p-fries', quantity: 1 }, { productId: 'p-cola', quantity: 2 }], discountPercent: 20, promoted: true, active: true } });
    const pub = (await admin.db.doc(`publicBranches/${IDS.branchA}/combos/${r.combo.id}`).get()).data()!;
    expect(pub.discountPercent).toBe(20);
  ...
  it('a combo order is priced server-side from current prices, reserves member stock, releases it on rejection', async () => {
    ...
    const pct = combos.docs[0]!.data().discountPercent as number;
    // Members: 2×2800 + 1200 + 2×800 = 8400, discounted per combo with floor rounding.
    const unit = Math.floor((8400 * (100 - pct)) / 100);
    ...
    expect(order.lines[0].comboDiscountPercent).toBe(pct);
